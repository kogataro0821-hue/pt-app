...
- データは残す。削除は管理者が明示的に「完全削除」を選んだときのみ実行（§46「データを消す処理は慎重に」）✅ 実装済み

**完全削除でやっていること（追加仕様: 契約者の完全削除）**

| 歯止め | 中身 |
|---|---|
| 事前に量を見せる | 「記録のある日: ◯日ぶん」「変更履歴: ◯件」を数えてから確認画面を出す |
| 契約者IDの手入力 | ぴったり一致するまでボタンを押せない。「本当によろしいですか？」は3回目から読まれないため |
| 消さない道を同じ画面に置く | 「無効にする」で足りることを、削除の画面で示す |
| 下から順に消す | 親を先に消すと、途中で失敗したとき「どの契約者のものか分からない孤児」が残る |
| やり直せる | 「いま在るものを数えて消す」だけなので、途中で止まっても再実行すれば残りから続く |
| 失敗を隠さない | 「消えたぶんは元に戻りません」と明示する |

> ★ **事前エクスポートは実装していません。** 設計時は「二段階確認 + 事前エクスポート」と書いていましたが、
> 実際に使う場面（お試し用の契約者を消す）では書き出す先が無く、手間だけが増えると判断しました。
> 必要になったら足せます（消す前に全部読んでいるので、そこからJSONを作るだけです）。

> ★ **ログインアカウント（Firebase Auth）は、アプリからは消せません。**
> 他人の Auth アカウントの削除には Admin SDK が要り、サーバーを持たない構成では置き場所がありません。
> Firebase の管理画面から人が手で消します。消すまで `{契約者ID}@pt-app.local` は使われたままなので、
> **同じ契約者IDで作り直せません。** 削除後の画面でそう伝えます。

---

### 6.7 会員ランク（追加仕様: 会員ランク）
...
| **DIAMOND / CROWN / CROWN AMBASSADOR** | **管理者が契約者ごとに条件を決める**（下記） |

**DIAMOND から先の条件**（追加仕様: 会員ランク）

契約者ごと・ランクごとに、管理者が3つ選んで決めます。
...
例）「運動記録・連続・30日」。**決めていないランクへは、記録がいくらあっても上がりません**（目標を渡していないため）。

- **順番を飛ばしません。** 運動だけ達成しても RUBY より先へは行きません。ただし条件が同時にそろえば一度に複数段上がります
- **自動での降格はありません。** 記録が減ってもランクは下がりません
- **管理者は「下げる」ことだけできます。** 上げるのは条件を満たしたときだけです。設定画面から好きなランクにはできません（条件を決めた意味がなくなるため）。下げ先は、いまより下ならどこでも選べます
- **確定するのはトレーナーです。** 条件を満たすと契約者の画面に「トレーナーの確認をお待ちください」、管理者の画面に「昇格させる」ボタンが出ます
  - 契約者に `rank` の書き込みを許すと、アプリを迂回して直接 CROWN にできます。`clients` の update で契約者に許した項目に `rank` は入っていません
- **数え方**: 日ドキュメントの `hasMeals` / `hasExercise`（カレンダーの印のために元からあるもの）を1回読んで数えます。下位コレクションは開きません
- **昇格の目印**: 契約者の画面で数えた結果を `extra.rankReady` に書き残し、管理者の契約者一覧に**追加の読み取り0回**でバッジを出します。目印は印が出るだけで、ランクは上がりません
- **契約者一覧の表示**: 整理番号・ランク・契約者ID・目標kcal・評価モード
- **会員証の形**: クレジットカードと同じ比率（85.60 × 53.98mm）。中身はロゴ・整理番号・ランク・表示名・入会年月の5つだけ。進み具合と累計はカードの外に出す
- **置き場所**: カレンダーの**上**
- **動き**: DIAMOND から上は、光の帯が横切り、きらめきがまたたく（星の色はカードに合わせる。DIAMOND は青白、CROWN 系は金）。動かすのは `transform` と `opacity` だけ。`prefers-reduced-motion: reduce` の端末では動かさず、きらめきは止まったまま出す
- **初期ランク**: 契約者を作るときに選べる（既定 PLATINUM）。これは「昇格」ではなく**開始地点**。作ったあとに上げることはできない
  - **★ 初期ランクを指定して作れる契約者は、システム全体で1人だけです。**
    誰でも好きな段から始められるなら、ランクは「頑張った印」ではなく「作るときに選ぶ飾り」になります。
    トレーナー自身のアカウントのような、本当の例外1つのためだけに開けてあります
  - 枠が空いているかは `clients` の `rankSeeded` で数えます。**回数は持ちません。**
    数だけ持つと、枠を使った契約者を消したあとに合わなくなり、二度と使えなくなります。
    実在する契約者で数えているので、**その契約者を削除すれば枠は空きます**
  - 作成画面では、枠が埋まっていると欄が使えなくなり、**誰が使っているか**を表示します。
    調べられなかったときは**安全側に倒して**選べなくします（分からないまま2人目を作らせない。
    作れなかっただけなら開き直せば済みますが、2人目を作ってしまうと戻せません）
  - 画面だけでなく `createClient` でも確かめます（画面の作りはあとから変わるため）。
    断るときは**1件も書きません**（作りかけの契約者を残さない）
- **会員整理番号**: 契約者を作ったときに「いまある番号の最大+1」で入ります（`0001` から）。
  - **既にいる契約者には、自動では振りません。** 動作確認用のアカウントや、トレーナー自身のアカウントには
    番号を振りたくない／別の番号にしたい、ということがあるためです
  - 契約者編集画面で手で入れられます。**`0` も使えます**（トレーナー自身のアカウントを `0000` にする、など）
  - 空欄にすると「未採番」。会員証には `No. ----` と出ます
  - 一度決めた番号は、なるべく変えません（会員証の番号が変わると別人の証になります）

---

### 6.8 お知らせ欄（追加仕様: お知らせ欄）

契約者の画面に、ベル（右上）とお知らせの一覧（`/c/{cid}/notices`）を置きます。

**★ お知らせは、人が書くのではなくアプリが自動で作ります。**

「管理者が書く欄」にすると、忙しい日は書かれません。書かれない欄は、契約者もやがて見なくなります。
起きたこと（登録・昇格・コメント）から作れば、必ず出ます。

| いつ | 出るお知らせ | 作る場所 |
|---|---|---|
| 契約者を作ったとき | 登録が完了しました | `createClient` |
| 昇格させたとき | 〈ランク〉になりました | `setClientRank`（**上げたときだけ**） |
| コメントを保存したとき | トレーナーからコメントが届きました | `NotesSection`（その日へのリンク付き） |
| アプリを配ったとき | アプリが新しくなりました | **アプリ同梱の定数** |

- **降格では出しません。** 降格をアプリが通知するのは、トレーナーの仕事を奪う形になります。伝えるなら人の言葉で伝える話です
- **同じ日のコメントは、何度直しても1件。** 目印を日付から作っています（`comment-{date}`）。直すたびに増えると、ベルの数字ばかり大きくなって中身が読まれません
- **お知らせを出せなくても、コメントは保存されています。** 通知は後回しで、書いた言葉を失うほうがずっと困ります

**置き場所と、無料枠**

| 種類 | 置き場所 | 読み取り |
|---|---|---|
| 個人あて | `clients/{cid}.notices`（配列・直近30件） | **0回**（契約者ドキュメントは元から読んでいる） |
| 全員向け | アプリの中の定数（`appNotices.ts`） | **0回**（Firestore に無い） |
| 既読の印 | `clients/{cid}.extra.noticeReadAt`（ミリ秒1つ） | 同上 |

下位コレクションにすると、画面を開くたびに読み取りが増えます。契約者ドキュメントはカレンダーを開いた時点で
すでに読んでいるので、その中に入れておけば**お知らせのための通信は0回**です。
そのかわり無限には溜められないので、30件で切ります。

**★ Rules は1行も足していません。**

`clients` の update で契約者に許した項目の一覧（`displayName` / `memo` / `extra` / `aiConsent` /
`passwordChangedAt` / `updatedAt`）に `notices` は入っていません。
**足すだけで自動的に管理者専用**になります。会員ランクのときと同じ仕組みです。

- 「トレーナーからコメントが届きました」を本人が作れると、お知らせは記録ではなく飾りになります
- 届いたお知らせを本人が消せるのも困ります（`notices: []` も弾かれます）
- **既読だけは本人が付けます。** そのため `extra` の中に置いています（`extra` は契約者が書ける唯一の自由欄）

この「一覧に入っていないから安全」という性質は、一覧に1語足した瞬間に消えます。しかも Rules の差分は
「許可を足した」ようにしか見えません。`firebase/tests/coverage.test.ts` が一覧を機械的に見張ります。

**既読は「1件ずつ」ではなく「いつまで読んだか」の時刻1つ**で持ちます。1件ずつ持つと、既読の印が
お知らせと同じ数だけ増えます。契約者が書ける場所は絞ってあり、小さく保ちたいところです。
一覧の「すべて閉じる」でベルの数字が消え、お知らせ自体は残ります（あとから読み直せます）。

**管理者が他人の画面で開いても、既読にはしません。** 様子を見に来ただけで既読になると、
  ...
  match /databases/{database}/documents {
    ...
    match /clients/{cid} {
      ...
      match /audits/{aid}     { allow read:   if isAdmin();
                                ...
                                allow update: if false;            // 書き換えは誰にも不可
                                allow delete: if isAdmin(); }      // 完全削除のときだけ
    }
...
| 監査 | 管理者が契約者データを編集した場合も `audits` に記録（**更新は誰にも不可**／削除は管理者のみ ← 契約者の完全削除のため） |
...
> ★ **登録依頼にAIの下書きを付ける機能は、作ったうえで取りやめました（2026-08-30）。**
>
> 動くところまで作りましたが、**無料枠を1日で使い切りました。**
> 管理者の下書き作りに使うぶんだけ、契約者が記録に使うぶんが減ります。
> **順番が逆です。** 契約者が食事を記録できることのほうが、この機能の目的そのものです。
>
> やめた判断の理由を残しておきます。同じ発想はまた出てきます。
>   ・登録依頼は「管理者が調べて手で打つ」で成立している（無いと困る機能ではない）
>   ・一方、契約者側の「文章から / 写真から / 成分表示から」は、無いと記録が止まる
>   ・限りある枠は、**止まると困るほうに使う**
>
> 復活させるなら、有料枠にするか、同じ食材名の結果を覚えておいて
> 2回目は聞かない仕組みを先に作ってからにします。

**★ 同意していないときは、AIの入口（文章から／写真から／成分表示から）を出しません。**
そのかわり、**出ない理由をその場に書きます。**

- ボタンが無いのと機能が無いのは、使う側からは区別が付きません。実際に、作った本人が
  「入口が消えた」と思って詰まりました。契約者ならなおさらです
- 契約者本人には「設定を開く」のリンクも出します。「同意が要ります」だけでは、どこで同意するのか分かりません
- 管理者にはリンクを出しません（**同意はご本人だけが行えます**。押しても何もできないリンクは、代われると誤解させます）
- 説明文の版が上がって取り直しが要るときは、そう書きます
- **中継役（Worker）が未設定のときは、何も言いません。** 同意しても入口は出ないので、
  案内どおりに同意させたうえで出ないのは、いちばん困らせる形です
- 食事が何件あっても、案内は最初の1つだけ（同じ文が何度も並ばないように）
...
| 個 / 枚 / 本 / パック | `Food.unitConversions` から取得（**実装済み**。追加仕様: 単位換算） |
| 杯 / 食 | **扱わない。** 量が人によって違い、「1杯」が誰の茶碗かを決めようがない |
| 大さじ | 15ml → 密度換算（未実装） |
| 小さじ | 5ml → 密度換算（未実装） |
...
| **追加仕様: 未処理の登録依頼の数** | 管理者の「登録依頼」の横に、未処理の件数を出す。0件のときは何も出さない。数だけを問い合わせる（`getCountFromServer`）ので読み取りは1回。一覧を読んだあとは、その数をそのまま使う | 依頼は契約者の操作で静かに積まれ、開かないかぎり気づけない。気づかないあいだ、その食材は栄養値0のまま集計され、**数字を根拠にした指導ができない日が黙って増える** |
| **追加仕様: 仮の栄養値** | マスタに**無い**食材にかぎり、契約者が100gあたりの kcal/P/F/C を入れられる。その値は合計に入るが「うち仮」として分けて表示し、管理者がマスタに登録した時点で置き換わる。マスタに**ある**食材には入力欄を出さない | 記録した日の合計が0のままだと続かない。一方で「白米が人によって156kcalだったり200kcalだったり」は絶対に避ける必要がある。**値がぶつかりうる場所には入力欄を出さない**という形で両立させた |
| **追加仕様: 契約者の完全削除** | 管理者が、契約者と記録をすべて削除できる。事前に量を表示し、契約者IDの手入力を求める。下位から順に消し、途中で止まってもやり直せる。ログインアカウントだけは Firebase の管理画面で手動 | 「無効にする」だけでは、お試し用に作った契約者が永久に残る。§6.6 に書いてあったが未実装だった |
| **追加仕様: 会員ランク** | PLATINUM→RUBY→SAPPHIRE→EMERALD→DIAMOND→CROWN→CROWN AMBASSADOR の7段階。カレンダーの下に会員証を出す（整理番号・会員ID・RANK・次のランクまでの進み具合）。RUBY〜EMERALD は記録の量で条件を判定し、**確定するのはトレーナー**。DIAMOND から先はトレーナーが直接設定。自動での降格は無し | 食事の記録は効果が出るまで数週間かかり、その間ずっと「面倒なだけ」に見える。そこに目に見える手応えを置くための仕掛け |
| **追加仕様: 筋肉量** | 日の記録に**筋肉量（kg）**を足す。体重・体脂肪率と同じ扱いで、目標値も設定でき、体重の推移の画面にもグラフが出る。**記録が1件も無くてもグラフの枠は出す**（当初は隠していたが、「機能が入っていない」と受け取られたため改めた）。`days` の項目を1つ増やすだけで、**Rules の変更なし**（days は禁止する項目を名指しする形のため） | **体重と体脂肪率からは計算で出せない。** 体脂肪率から分かるのは除脂肪量（骨・水分を含む）であって筋肉量ではない。体組成計が別に出す数字なので、別の欄として持つ。増量期は体重より筋肉量のほうが見たい数字になる |
| **追加仕様: メニュー** | 画面右上を三本線のメニューにまとめ、その中に「設定 / パスワードの変更 / ログアウト」を入れる。**管理者・契約者のどちらにも同じ形で出す**。設定の行き先だけが役割で変わる（契約者本人は `/c/{id}/settings`、管理者は「いま見ている契約者」の `/clients/{id}/settings`）。行き先の無い画面では設定の項目を出さない。カレンダー上部にあった管理者向けの「設定」は、行き先が二重になるので取り除いた | **画面のいちばん押しやすい所に、いちばん押してほしくない操作（ログアウト）が常時出ていた。** スマホで戻るつもりの指がかすれば、それだけで落ちる。一段しまえば、うっかりは起きない。探す手間は覚えれば終わる手間で、失った入力は戻らない |
| **追加仕様: 単位換算** | 食品マスタに「1個 = 50g」の換算を持たせ、食材の入力欄に単位のプルダウン（g / 個 / 枚 / 本 / パック）を出す。**換算を決めるのは管理者だけ**で、契約者は選ぶだけ。記録に保存するのは換算後のグラムで、「2個」は表示用の控え（`enteredAs`）として別に持つ。登録画面では入力中に「1個 ≒ 71kcal」を出して桁の打ち間違いを拾う。`foods` も `meals` も項目の名指し制限が無いため**Rules の変更なし** | 設計書 §10.5 と §16.2（「卵1個（個→g換算）」）に最初からあったが、未実装のままだった。**卵は殻が1〜2割ある**ので、量りに乗せた60gをそのまま入れると2割多く計算され、しかも画面に何も出ない。換算を各自に入れさせるとAさんの卵1個が50g・Bさんが60gになり、個人マスタを廃止したのと同じ問題が戻る。「杯」「食」「大さじ」は、正確そうに見えて実は目分量になるので対象から外した |
| **追加仕様: マスタ更新の反映** | 食品マスタの「最終更新」だけを入れた1件の書類（`config/foods`）を置き、契約者は食材の入力を開くたびに**それ1件だけ**を読む。前回と変わっていたときだけ、マスタ全件を読み直す。読めなかったときは覚えている一覧をそのまま使い、印を付けるのに失敗しても保存そのものは成功にする。`config` は既存の枠（管理者が書く・全員が読む）なので**Rules の変更なし** | 食品マスタは開いた最初の1回だけ読んで、あとは覚えていた。ところが**それを捨てる道が無かった。** 管理者が食材や換算を登録しても契約者のアプリには伝わらず、JavaScriptが動き直すまで古いまま。ホーム画面から開いたアプリは、他のアプリに切り替えて戻っただけでは動き直さないので、**何日も古い一覧のまま使い続ける**ことになる。契約者から見ると「登録依頼を出したのに、いつまでも反映されない」。毎回全件を読み直すと無料枠が尽きるので、1件だけ読む形にした |
| **追加仕様: 版の表示** | 右上のメニューの最下部に `ver 1.0.0 / コミット / 組み立て時刻` を小さく出し、その下に「最新に更新する」を置く。押すと Service Worker を外し、Cache を捨て、入口のファイルをキャッシュ無視で取り直してから開き直す。値はビルド時に vite の define で埋め込み、埋め込みが無くても落ちない（テストのときは既定値）。**Rules の変更なし** | **パソコンでの確認手段が無いため、直したものが端末に届いたかを確かめる方法が無かった。** 「直したはずなのに直っていない」のか「まだ古いアプリが動いている」のかが区別できず、食品マスタの更新が届かない件では、動いているコードを何度も疑うことになった。コミットは GitHub Actions の一覧に出る文字列と同じなので、緑になった行と画面の隅を見比べるだけで済む。番号だけでは上げ忘れれば嘘になるので、自動で必ず変わる時刻とコミットを併記する |
| **追加仕様: 名前の重複に印** | 食品マスタの一覧で、名前や別名がぶつかっている食材に印を出す。一覧の上に件数、各行にぶつかっている相手の名前、行の左に帯。ぶつかりの判定は照合キーで行い、**検索で絞る前の全件**から出す。自分の本名と別名が同じキーになる場合は、ぶつかり扱いにしない。**Rules の変更なし** | 「卵」が2件あり、片方は F 10.2、もう片方は古い F 1.2 だった。管理者は新しいほうを直していたのに、契約者の画面には**古いほうの数字が出ていた。** `findExactFood` は当たった中の先頭を黙って返すため、どちらが選ばれるかは並び順しだいで、画面には何も出ない。トレーナーが数字を根拠に指導するアプリで「どの数字が使われるか分からない」はあってはならない。原因の特定に何往復もかかった |
| **追加仕様: ログインポイント** | 契約者がアプリを開いた日に既定100pt。**1日の区切りは JST 朝4時**（深夜に記録する人にとって午前1時はまだ前日のため）。額は管理者が契約者ごとに設定（`pointsDailyAmount`、既定100）。管理者は個人／選択／全員にポイントを付与・減算でき、そのときお知らせを書ける。**貯めたポイントは実物と交換する**（リアルで直接交渉）ので、減算も管理者の操作として用意。残高はマイナスにしない。置き場所は `clients/{cid}` の決まった項目（`points` / `pointsDailyAmount` / `pointsLastDate` / `pointsTotalDays`）で、**`extra` ではない** | **ポイントが実物と交換できる＝本物の価値を持つ数字**になったため、飾りのバッジと同じ守り方ではいけない。`extra` は契約者が何でも書ける唯一の自由欄なので、そこに置くと開発者ツールから残高を書き換えられる。そこで置き場所を決まった項目に移し、**Rules で「今日ぶんを1回受け取る」だけを条件付きで許可**した（サーバー時刻の今日・未受け取り・設定額ぴったり・累計+1）。付与額は管理者しか書けないので「額を上げてから受け取る」も塞がる。**これに伴い Rules を変更**（貼り直しが必要）。連続方式にしなかったのは前と同じ理由（休んだ日にそのまま離れるため）。節目ボーナスは廃止（管理者が自由に配れるようになり、「+設定額ぴったり」という単純な条件のほうが Rules に隙間ができないため）。管理者の代理閲覧では配らない |

### 配布の手順（毎回これを通す）

★ ここは、実際に抜けた手順を書き留めたものです。

1. 変更を入れ、`npm run verify` と `npm run build` を通す
2. **画面に出る変更なら、`apps/web/src/features/notices/appNotices.ts` の
   いちばん上に、お知らせを1件足す**（1回の配布につき1件）
3. ZIP に「ルール変更の有無」とコミット名を必ず添える
4. Push したあと、Actions が緑になるのを確認する

★ 2 を5回続けて忘れました。

  8/29 にお知らせ欄を作ったあと、筋肉量・メニュー・単位換算・
  マスタの反映・版の表示と配って、お知らせは1件も増えませんでした。
  使う人から見れば「更新されない欄」です。そうなればもう見に来ません。
  **お知らせ欄そのものが死にます。** 機能を作ったこと自体が無駄になります。

  テストでは捕まえられません（配ったかどうかをテストは知らない）。
  手順として書いておくしかない種類の抜けです。
...
| E2E | **Playwright**（Phase 11C・未着手） | ログイン→記録→依頼→登録→反映→確定 | 主要シナリオ |

### 16.2 計算テスト（§43）

```
□ 白米150g / 180g          → per100gからの換算が正確か
☑ 卵1個（個→g換算）        → unitConversions 経由（追加仕様: 単位換算で実装）
