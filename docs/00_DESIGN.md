...
| **追加仕様: 筋肉量** | 日の記録に**筋肉量（kg）**を足す。体重・体脂肪率と同じ扱いで、目標値も設定でき、体重の推移の画面にもグラフが出る。記録が1件も無いうちはグラフを出さない。`days` の項目を1つ増やすだけで、**Rules の変更なし**（days は禁止する項目を名指しする形のため） | **体重と体脂肪率からは計算で出せない。** 体脂肪率から分かるのは除脂肪量（骨・水分を含む）であって筋肉量ではない。体組成計が別に出す数字なので、別の欄として持つ。増量期は体重より筋肉量のほうが見たい数字になる |
