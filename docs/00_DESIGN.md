...
マスタの名前は、**全員で使う辞書**であって個人の情報ではありません。
「すでにある食材と同じか」を判断させるために必要なので、名前だけを渡します。
...
| **追加仕様: 登録依頼のAI** | 管理者が登録依頼を処理するとき、**ボタンを押したときだけ** AI に下書きを作らせる。①100gあたりの推定値 ②表記ゆれ（別名）の案 ③すでにある食材と同じかの判断。AI に送るのは**食材名とマスタにある食材の名前だけ**。返ってきたものは決定論的な関門（`refineFoodDraft`）を必ず通し、**マスタには一切書かない**（人が確かめて保存したときだけ確定） | マスタの1件は全契約者の集計に効く。ここだけは他より一段きつく守る必要がある。一方で、依頼のたびに管理者が成分表を調べて手で打つのは続かない |
