...
function toClient(id: string, data: Record<string, unknown>): Client {
  ...
  return {
    ...
    pointsDailyAmount: num(data.pointsDailyAmount) ?? DEFAULT_DAILY_POINTS,
    pointsLastDate: str(data.pointsLastDate) ?? '',
    pointsTotalDays: num(data.pointsTotalDays) ?? 0,
...
function toFirestore(client: Client): Record<string, unknown> {
  return {
    ...
    pointsDailyAmount: client.pointsDailyAmount,
    pointsLastDate: client.pointsLastDate,
    pointsTotalDays: client.pointsTotalDays,
